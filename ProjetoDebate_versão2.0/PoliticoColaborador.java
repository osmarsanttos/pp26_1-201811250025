...
public class PoliticoColaborador implements Prototype<PoliticoColaborador> {
    ...
    public void adicionarEleitor(ObservadorEleitor eleitor) { eleitores.add(eleitor); }
    public void removerEleitor(ObservadorEleitor eleitor) { eleitores.remove(eleitor); }
    ...
    public Eleitor obterEleitorPorNome(String nomeEleitor) {
        for (ObservadorEleitor observador : eleitores) {
            if (observador instanceof Eleitor) {
                Eleitor eleitor = (Eleitor) observador;
                if (eleitor.getNome().equalsIgnoreCase(nomeEleitor)) return eleitor;
            }
        }
        return null;
    }
    ...
    public void notificarEleitores() {
        ...
        if (eleitores.isEmpty()) System.out.println("Nenhum eleitor cadastrado para receber notificação de " + nome + ".");
        for (ObservadorEleitor eleitor : eleitores) eleitor.receberNotificacao(mensagem);
    }
    ...
    public void listarEleitores() {
        ...
        for (ObservadorEleitor observador : eleitores) {
            if (observador instanceof Eleitor) System.out.println("- " + ((Eleitor) observador).getNome());
        }
    }

    @Override
    public PoliticoColaborador clonar() {
        PoliticoColaborador copia = new PoliticoColaborador(this.nome, this.partido, this.numero, this.mediador);
